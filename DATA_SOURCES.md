...
CAKRAWALA's **code** is [MIT](LICENSE)-licensed. **The MIT grant covers the source code only — it does NOT extend to third-party data or visual assets.** Every third-party source keeps its own license and terms. This file documents the live and bundled data sources; bundled 3D-model provenance is recorded in [`public/models/README.md`](public/models/README.md).
...
| **BMKG** (TEWS gempabumi JSON, prakiraan cuaca API) | Indonesia earthquakes (latest, M5+, felt, tsunami potential) and 3-hourly city forecasts in the Indonesia command center | Public data / public API of BMKG; attribution required | "Data gempabumi & prakiraan cuaca: BMKG (bmkg.go.id)" |
...
| **ReliefWeb (UN OCHA)** | Ongoing Indonesia disasters — only when `RELIEFWEB_APPNAME` is set | ReliefWeb API terms (registered appname) | "ReliefWeb" |
| **OpenFreeMap** (Liberty / Positron styles) + **Esri World Imagery** | Keyless basemaps of the 2D GIS map | OpenMapTiles + ODbL (OSM); Esri terms as above | Rendered by MapLibre's attribution control |
| **OurAirports** | 624 Indonesian airports (bundled snapshot) for the GIS map and spatial queries | Public domain | "OurAirports" |
| **wilayah.id** (Kemendagri codes) | Province / regency / district / village codes and names (bundled snapshot) | MIT API over Kemendagri public data | "Kemendagri via wilayah.id" |
...
| **Indonesian city CCTV portals** (Yogyakarta, Bandung, Banda Aceh, Palembang, Salatiga, Bengkulu, Banjarmasin, Semarang, Sidoarjo, Pekalongan, Depok) + BPJT toll roads | CCTV camera catalogs + live HLS streams, Indonesia | Public city CCTV published for public viewing; Banda Aceh streams are CC BY 4.0 | Each city portal (cctv.jogjakota.go.id, pelindung.bandung.go.id, cctv.bandaacehkota.go.id, cctv.palembang.go.id, cctv.salatiga.go.id, cctv.bengkulukota.go.id, cctv.banjarmasinkota.go.id, pantausemar.semarangkota.go.id, pantaulalindishub.sidoarjokab.go.id, cctv.pekalongankota.go.id, dishub.depok.go.id, bpjt.pu.go.id/cctv) |
| **Hong Kong Transport Department** (traffic snapshot images) | CCTV camera catalog + frames, Hong Kong | [DATA.GOV.HK terms](https://data.gov.hk/en/terms-and-conditions) | "Transport Department, HKSAR Government — DATA.GOV.HK" |
| **Singapore LTA traffic images** (data.gov.sg) | CCTV camera catalog + frames, Singapore | [Singapore Open Data Licence v1.0](https://data.gov.sg/open-data-licence) | "Contains information from Traffic Images accessed from data.gov.sg, made available under the Singapore Open Data Licence version 1.0" |
| **Fintraffic Digitraffic weather cameras** | Road weather camera catalog + frames, Finland | [CC BY 4.0](https://creativecommons.org/licenses/by/4.0/) | "Fintraffic / digitraffic.fi, CC BY 4.0" |
...
> If you use CAKRAWALA commercially, delete `src/data/local_data/telegeography_submarine_cables/` (or obtain a commercial license from TeleGeography). It is one self-contained folder; the rest of the app runs without it.
