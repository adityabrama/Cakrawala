...
CAKRAWALA is a local-first client for **public** data. It is built for exploration, demos, and learning — not as a hardened production service. This document explains the security model so you can run it safely and report issues responsibly.
...
1. **Google Maps API key** — loads Photorealistic 3D Tiles directly and powers CAKRAWALA place search. **Restrict it** (HTTP referrer + API restriction to the required Google APIs) in the Google Cloud Console. An unrestricted key in a public deployment can be abused and billed to you.
...
- **Pinokio LAN and Cloudflare sharing are refused.** The current supported
  ...
  sharing value is therefore discarded rather than honored, and CAKRAWALA starts on
